...
func (c *ExecutionHelper) CallCapability(ctx context.Context, request *sdkpb.CapabilityRequest) (*sdkpb.CapabilityResponse, error) {
	// Block user workflows from calling system-only capabilities directly.
	// These are internal plumbing (e.g. CRE -> enclave dispatch) and must not
	// be invocable from workflow steps.
	if isSystemCapability(request.Id) {
		return nil, fmt.Errorf("capability %q is system-only and cannot be called from a workflow", request.Id)
	}
