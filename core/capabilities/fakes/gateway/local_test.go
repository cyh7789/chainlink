...
func TestListenForTriggerPayload_HappyPath(t *testing.T) {
	...
	// Sign the body with a fresh ECDSA key and construct the Bearer JWT.
	signingKey := generateTestKey(t)
	authHeader := buildJWT(t, signingKey, body)

	// Send the POST request to the gateway.
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost,
		fmt.Sprintf("http://127.0.0.1:%d/", port), bytes.NewReader(body))
	require.NoError(t, err)
	...
	// The authorized key must reflect the signer's Ethereum address.
	require.NotNil(t, res.payload.Key)
	expectedAddr := strings.ToLower(crypto.PubkeyToAddress(signingKey.PublicKey).Hex())
